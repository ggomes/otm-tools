...
%         end
        ...
        function X = get_data(this)
            X = this.api.output.get_data;
        end

        function X = get_file_names(this)
            X = this.api.output.get_file_names;
        end
        ...
        function request_lanegroups(this,prefix,output_folder)
            this.api.output.request_lanegroups(prefix,output_folder);
        end

        function request_links_flow(this,varargin)
            % varargin = outDt,link_ids,commodity_id,prefix,output_folder
            [outDt,link_ids,commodity_id,prefix,output_folder] = this.process_input_A(varargin);
            if isempty(prefix) || isempty(output_folder)
                this.api.output.request_links_flow(commodity_id,link_ids,outDt);
            else
                this.api.output.request_links_flow(prefix, output_folder, commodity_id, link_ids, outDt);
            end
        end

        function request_links_veh(this,varargin)
            % varargin = outDt,link_ids,commodity_id,prefix,output_folder
            [outDt,link_ids,commodity_id,prefix,output_folder] = this.process_input_A(varargin);
            if isempty(prefix) || isempty(output_folder)
                this.api.output.request_links_veh(commodity_id,link_ids,outDt);
            else
                this.api.output.request_links_veh(prefix, output_folder, commodity_id, link_ids, outDt);
            end
        end

        function request_lanegroup_flw(this,varargin)
            % varargin = outDt,link_ids,commodity_id,prefix,output_folder
            [outDt,link_ids,commodity_id,prefix,output_folder] = this.process_input_A(varargin);
            if isempty(prefix) || isempty(output_folder)
                this.api.output.request_lanegroup_flw(commodity_id,link_ids,outDt);
            else
                this.api.output.request_lanegroup_flw(prefix, output_folder, commodity_id, link_ids, outDt);
            end
        end

        function request_lanegroup_veh(this,varargin)
            % varargin = outDt,link_ids,commodity_id,prefix,output_folder
            [outDt,link_ids,commodity_id,prefix,output_folder] = this.process_input_A(varargin);
            if isempty(prefix) || isempty(output_folder)
                this.api.output.request_lanegroup_veh(commodity_id,link_ids,outDt);
            else
                this.api.output.request_lanegroup_veh(prefix, output_folder, commodity_id, link_ids, outDt);
            end
        end

        function request_path_travel_time(this,varargin)
            % varargin = outDt,subnetwork_ids,prefix,output_folder
            [outDt,subnetwork_ids,prefix,output_folder] = this.process_input_B(varargin);
            
            it = subnetwork_ids.iterator;
            while it.hasNext
                subnetwork_id = java.lang.Long(it.next);
                if isempty(prefix) || isempty(output_folder)
                    this.api.output.request_path_travel_time(subnetwork_id,outDt);
                else
                    this.api.output.request_path_travel_time(prefix,output_folder,subnetwork_id,outDt);
                end
            end
                
        end

        function request_subnetwork_vht(this,varargin)
            % varargin = outDt,subnetwork_ids,commodity_id,prefix,output_folder
            [outDt,subnetwork_ids,commodity_id,prefix,output_folder] = this.process_input_C(varargin);
            it = subnetwork_ids.iterator;
            while it.hasNext
                subnetwork_id = java.lang.Long(it.next);
                if isempty(prefix) || isempty(output_folder)
                    this.api.output.request_subnetwork_vht(commodity_id,subnetwork_id,outDt);
                else
                    this.api.output.request_subnetwork_vht(prefix,output_folder,commodity_id,subnetwork_id,outDt);
                end
            end
        end

        function request_vehicle_events(this,varargin)
            % varargin = commodity_id,prefix,output_folder
            [commodity_id,prefix,output_folder] = this.process_input_D(varargin);
            if isempty(prefix) || isempty(output_folder)
                this.api.output.request_vehicle_events(commodity_id);
            else
                this.api.output.request_vehicle_events(prefix,output_folder,commodity_id);
            end
        end

        function request_vehicle_class(this,prefix,output_folder)
            if nargin<3
                this.api.output.request_vehicle_class();
            else
                this.api.output.request_vehicle_class(prefix,output_folder);
            end
        end

        function request_vehicle_travel_time(this,prefix,output_folder)
            if nargin<3
                this.api.output.request_vehicle_travel_time();
            else
                this.api.output.request_vehicle_travel_time(prefix,output_folder);
            end
        end

        function request_actuator(this,actuator_id,prefix,output_folder)
            this.api.output.request_actuator(prefix,output_folder,actuator_id);
            this.api.output.request_actuator(actuator_id);
        end

        function request_controller(this,controller_id,prefix,output_folder)
            this.api.output.request_controller(prefix,output_folder,controller_id);
        %     this.api.output.request_controller(controller_id);
        end
    ...
    methods(Access=private)
        ...
        function [outDt,link_ids,commodity_id,prefix,output_folder] = process_input_A(this,varargin)
            % varargin = outDt,link_ids,commodity_id,prefix,output_folder
            ...
            if numel(inputs)==4
                error('varargin number of arguments')
            end
            
            outDt = Matlab2Java(inputs{1},'Float');
            
            if numel(inputs)<2 || isempty(inputs{2})
                link_ids = this.api.scenario.get_link_ids;
            else
                link_ids = Matlab2Java(inputs{2},'LongList');
            end
            
            if numel(inputs)<3 || isempty(inputs{3})
                commodity_id = [];
            else
                commodity_id = Matlab2Java(inputs{3},'Long');
            end
            ...
            if numel(inputs)<5
                ...
                prefix = inputs{4};
                output_folder = inputs{5};
            end
                
        end

        function [outDt,subnetwork_ids,prefix,output_folder] = process_input_B(this,varargin)
            % varargin = outDt,subnetwork_ids,prefix,output_folder
            ...
            if numel(inputs)<2 || isempty(inputs{2})
                subnets = Java2Matlab(this.api.scenario.get_subnetworks);
                subnetwork_ids = Matlab2Java([subnets.id],'LongSet');
            else
                subnetwork_ids = Matlab2Java(inputs{2},'LongSet');
            end
        ...
        function [outDt,subnetwork_ids,commodity_id,prefix,output_folder] = process_input_C(this,varargin)
            % varargin = outDt,subnetwork_ids,commodity_id,prefix,output_folder
        ...
        function [commodity_id,prefix,output_folder] = process_input_D(this,varargin)
            % varargin = commodity_id,prefix,output_folder
            ...
            if numel(inputs)<1 || isempty(inputs{1})
                ...
                commodity_id = Matlab2Java(inputs{1},'Long');
            end
